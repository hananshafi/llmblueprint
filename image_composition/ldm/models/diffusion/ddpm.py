...
class LatentDiffusion(DDPM):
    ...
    def differentiable_decode_first_stage(self, z, predict_cids=False, force_not_quantize=False):
        ...
        if hasattr(self, "split_input_params"):
            ...
            if isinstance(self.first_stage_model, VQModelInterface):
                print("entering first_stage_model VQModelInterface . . . . ")
                return self.first_stage_model.decode(z, force_not_quantize=predict_cids or force_not_quantize)
            else:
                print("entering first_stage_model NOOOO VQModelInterface . . . . ")
                return self.first_stage_model.decode(z)
                if self.first_stage_key=='inpaint':
                    return self.first_stage_model.decode(z[:,:4,:,:])
                else:
                    print("NOO first stage key inpaint   . . . ")
                    return self.first_stage_model.decode(z)
                # return self.first_stage_model.decode(z)

    @torch.no_grad()
    def encode_first_stage(self, x):
        print("entering encode first stage .. .")
        if hasattr(self, "split_input_params"):
            if self.split_input_params["patch_distributed_vq"]:
                ks = self.split_input_params["ks"]  # eg. (128, 128)
                stride = self.split_input_params["stride"]  # eg. (64, 64)
